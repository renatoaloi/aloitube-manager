...
export default function Videos() {
    ...
    const carregarVideos = async () => {
        try {
            ...
            showModal({
                title: 'Acesso Negado',
                message: 'Erro ao carregar os vídeos do canal. Verifique se o módulo de OAuth está conectado.',
                type: 'error'
            });
        } finally {
            setLoading(false);
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '30px', borderBottom: '1px solid var(--border)', paddingBottom: '15px' }}>
                <h1 style={{ fontSize: '32px', margin: 0, color: 'var(--text-h)', fontWeight: '900', letterSpacing: '-1px' }}>
                    Ativos <span style={{ color: 'var(--accent)', textShadow: 'var(--shadow-neon)' }}>Digitais</span>
                </h1>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                ...
                {loading ? (
                    <div className="glass-panel" style={{ textAlign: 'center', padding: '50px', color: 'var(--text)' }}>
    ...
    return (
        <div
            ...
            style={{
                ...
                borderLeft: isHovered ? '4px solid var(--accent)' : '4px solid var(--border)',
                padding: '16px',
        ...
        >
            {/* 1. Thumbnail Pequena */}
            <div style={{ flexShrink: 0, width: '160px', height: '90px', background: '#000', overflow: 'hidden', position: 'relative', borderRadius: '4px', border: '1px solid var(--border)' }}>
                <img src={vid.thumb} alt="Thumbnail" style={{ width: '100%', height: '100%', objectFit: 'cover', opacity: isHovered ? 1 : 0.7, transition: 'opacity 0.2s', filter: isHovered ? 'none' : 'grayscale(30%)' }} />
                <div style={{ position: 'absolute', bottom: 0, right: 0, background: 'rgba(3,7,18,0.9)', color: 'var(--accent)', fontSize: '10px', padding: '4px 6px', fontWeight: '800', fontFamily: 'var(--mono)' }}>
                    10:24
                </div>
            </div>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
                <h3 style={{ fontSize: '16px', margin: '0 0 6px 0', color: 'var(--text-h)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', fontWeight: '600' }}>
                    {vid.titulo}
                </h3>
                <p style={{ margin: '0 0 12px 0', fontSize: '13px', color: 'var(--text)', lineHeight: '1.4' }}>
                    {vid.resumo.slice(0, 100)}...
                </p>

                {/* Metadados */}
                <div style={{ display: 'flex', gap: '16px', fontSize: '11px', color: 'var(--text)', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                    <span style={{ display: 'flex', alignItems: 'center', gap: '4px', color: vid.visibilidade === "Público" ? 'var(--success)' : 'var(--warning)' }}>
                        {vid.visibilidade === "Público" ? "🌐" : "🔒"} {vid.visibilidade}
                    </span>
                    <span>📅 {vid.data}</span>
                    <span style={{ fontFamily: 'var(--mono)' }}>👁️ {vid.views}</span>
                    <span style={{ fontFamily: 'var(--mono)' }}>💬 {vid.comentarios}</span>
                </div>
            </div>

            {/* 4. Ações Rápidas (Ícones) */}
            <div style={{ display: 'flex', gap: '8px', borderLeft: '1px solid var(--border)', paddingLeft: '24px' }}>
                <ActionButton
                    icon={icons.download}
                    ...
                    onClick={async () => {
                        try {
                            ...
                            showModal({
                                ...
                                message: 'Não foi possível acionar o pipeline de download neste momento.',
                                type: 'error'
                            });
                        }
                    }}
