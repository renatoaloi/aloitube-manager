...
export default function Thumbnails() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(carregarThumbnails, 3000);
    return () => clearInterval(interval);
  }, []);

  const carregarThumbnails = async () => {
    try {
      const data = await ProcessingService.listarThumbnails();
      setThumbnails(data);
    } catch (error) {
      console.error("Erro ao carregar as thumbnails.", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const verSugestoes = async (taskId) => {
    try {
      const data = await ProcessingService.obterThumbnailDetalhes(taskId);
      showModal({
                ...
                >
                  ...
                  <td style={{ padding: "16px" }}>
                    {item.status === "CONCLUIDO" ? (
                      ...
                      >
                        <Lucide.Image size={14} /> EXIBIR THUMBNAILS
                      </button>
                    ) : (
